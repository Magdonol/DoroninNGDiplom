...
&НаСервере
Процедура ОбъемДисциплиныПриИзмененииНаСервере()
	Спр = Справочники.ОбъемыПромежуточнойАттестации.НайтиПоНаименованию(Объект.ОбъемДисциплины); 
	Если НЕ Спр.КоличествоРейтинговыхРабот = 0 Тогда  
		Рр = Спр.КоличествоРейтинговыхРабот; 
		Пока Рр > 0 Цикл
			О = Объект.ТЧ.Добавить(); 
			О.ВидАттестации =Перечисления.ВидыромежуточнойАттестации.рейтинговаяРабота; 
			Запрос = Новый Запрос; 
			Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
			               |	РабочаяПрограммаДисцилиныПреподавательскийСостав.Преподаватель КАК Преподаватель
			               |ИЗ
		...
		рез.Следующий(); 
		О.Преподаватель = рез.Преподаватель; 
		Рр = Рр - 1; 
		КонецЦикла; 
	...
	Если НЕ Спр.КоличествоТестов = 0 Тогда  
		Тест = Спр.КоличествоТестов; 
		Пока Тест > 0 Цикл
			О = Объект.ТЧ.Добавить(); 
...
&НаКлиенте
Процедура ОбъемДисциплиныПриИзменении(Элемент)
	ОбъемДисциплиныПриИзмененииНаСервере();
КонецПроцедуры
...
Процедура ТЧДатаПриИзменении(Элемент)
	...
	Если Стр.Дата > Объект.КонецСеместраДляТестов ИЛИ Стр.Дата < Объект.НачалоСеместраДляТестов Тогда 
		Сообщить("Дата проведения аттестационного мероприятия не может быть больше даты окончания для решения тестов"); 
		Стр.Дата = ""; 
	КонецЕсли; 
...
Процедура ПериодСессииПриИзмененииНаСервере()
	...
	Иначе 
		...
		Объект.НачалоСеметраДляТестов = Спр.НачалоСеметраДляТестов;
		Объект.КонецСеместраДляТестов = Спр.КонецСеместраДляТестов;
	КонецЕсли; 
