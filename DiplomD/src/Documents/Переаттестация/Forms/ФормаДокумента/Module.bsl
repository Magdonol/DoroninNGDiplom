...
Процедура ЗаявлениеНаПересдачуПриИзмененииНаСервере()
	...
	Запрос2.Текст = "ВЫБРАТЬ
	                ...
	                |ГДЕ
	                |	РабочаяПрограммаДисцилиныАттестация.Семестр = &Семестр
	                |	И РабочаяПрограммаДисцилиныАттестация.ПрофильНаправления = &ПрофильНаправления
	                |	И РабочаяПрограммаДисцилиныАттестация.Ссылка = &Ссылка";
	  Запрос2.УстановитьПараметр("Семестр",Объект.Семестр); 
	  Запрос2.УстановитьПараметр("ПрофильНаправления",Объект.НаправлениеПодготовки);
	  Запрос2.УстановитьПараметр("Ссылка",Стр.Дисциплина); 
	  Р = Запрос2.Выполнить().Выбрать(); 
	  Р.Следующий(); 
	  О.КоличествоЧасов = Р.КоличествоЧасов;
	  О.ФормаИтоговогоКонтроля = Р.ВидАттестации
