...
Процедура СеместрПриИзмененииНаСервере()
		...
		Для каждого стр из рез Цикл 
			Если ПустаяСтрока(Стр.Оценка) Тогда
...
Процедура СоздатьЗаявленияНаПереаттестациюНаСервере()
		...
		Для каждого строка из рез Цикл 
			Если пустаяСтрока(строка.Оценка) ИЛИ Строка.Оценка = Перечисления.ВидыОценокЗачет.НеЗачтено ИЛИ Строка.Долг = Истина Тогда 
			О = Док.ТЧ.Добавить(); 
